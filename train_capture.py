    ...
    while True:
        try:
            if video_feed.get_status():
                ...
                frame_change = cv2.GaussianBlur(cv2.cvtColor(
                ...
                frame_threshold = cv2.dilate(cv2.threshold(
                    frame_diff, 20, 255, cv2.THRESH_BINARY)[1], None, iterations=2)

                contours, _ = cv2.findContours(
                    frame_threshold.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                ...
                if motion:
                    ...
                    image_writer.add_to_queue(
                        path + str(time.time()) + '.jpg', frame)
                    last_motion = time.time()
                elif (time.time() - last_motion).total_seconds() / 60 >= 5:
